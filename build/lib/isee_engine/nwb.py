...
import uuid
import tempfile
from isee_engine.visualization.widgets import PlotWidget, MovieWidget
...
#     def add_to_general(self, f):
#         if self._hdf5_location is None:
#             scale = self.scale_range
#             self._hdf5_location = _set_scale(f['general'], 'scale', scale, self.dimension, self.unit, self.__class__.__name__)
#         else:
#             scale = self._hdf5_location
#             _set_scale(f['general'], 'scale', scale, self.dimension, self.unit, self.__class__.__name__)
...
# <<<<<<< HEAD
#     def add_to_stimulus(self, f):
#         return self._add_to_group(f, 'stimulus', self.__class__.group)
# =======
    def add_to_stimulus(self, f, compression='gzip', compression_opts=4):
        self._add_to_group(f, 'stimulus', self.__class__.group, compression=compression, compression_opts=compression_opts)
...
# <<<<<<< HEAD
#     def add_to_acquisition(self, f):
#         return self._add_to_group(f, 'acquisition', self.__class__.group)
# =======
    def add_to_acquisition(self, f, compression='gzip', compression_opts=4):
        self._add_to_group(f, 'acquisition', self.__class__.group, compression=compression, compression_opts=compression_opts)
# >>>>>>> e1b0ffd7248e02279411b99ef80de286c5f10fc0
    ...
    def add_to_processing(self, f, processing_submodule_name):
        ...
        if not processing_submodule_name in f['processing']:
...
# <<<<<<< HEAD
#                         
#             return_data = [cls.get_processing(f, subgroup_name, ii) for ii in range(len(f['processing/%s/%s' % (subgroup_name, cls.group)]))]
#             if len(return_data) == 1: return_data = return_data[0]
# =======
            return_data = {}
...
# >>>>>>> e1b0ffd7248e02279411b99ef80de286c5f10fc0
    ...
    def add_to_analysis(self, f, analysis_submodule_name):
        ...
        if not analysis_submodule_name in f['analysis']:
    ...
    def get_analysis(cls, f, subgroup_name, ii=None):
        if ii is None:
            return [cls.get_analysis(f, ii, subgroup_name) for ii in range(len(f['analysis/%s/%s' % (subgroup_name, cls.group)]))]
        else:
            ...
            if isinstance(scale, (Scale, DtScale, NullScale)):
                if scale._hdf5_location is None:
                    if  scale_group is None:
                        ...
                        curr_scale = _set_scale(scale_group, 'dimension_%s' % ii, scale.data, scale.dimension, scale.unit, scale.__class__.__name__)
...
# <<<<<<< HEAD
#         return _set_data(subgroup, dataset_group.name, self.data, scale_list, self.dimension, self.unit, metadata=self.metadata)
# =======
        _set_data(subgroup, dataset_group.name, self.data, scale_list, self.dimension, self.unit, metadata=self.metadata, compression=compression, compression_opts=compression_opts)
# >>>>>>> e1b0ffd7248e02279411b99ef80de286c5f10fc0
...
class SpikeTrain(Data):
    ...
    def __init__(self, data, **kwargs):
        ...
        if isinstance(scale, Scale):
            super(SpikeTrain, self).__init__(data, 'index', unit, scale, metadata)
        elif len(scale) == 0:
...
#     
#     
# def _set_data(parent_group, dataset_name, data, scales, dimension, unit, force=False, metadata={}):
#         
#     
# =======
def _set_data(parent_group, dataset_name, data, scales, dimension, unit, force=False, metadata={}, compression='gzip', compression_opts=4):
...
# >>>>>>> e1b0ffd7248e02279411b99ef80de286c5f10fc0
    ...
    for ii, scale in enumerate(scales):
        ...
        if check_dimension == 'None': check_dimension = None
        check_unit = scale.attrs['unit']
        if check_unit == 'None': check_unit = None
    ...
    dataset = dataset_group.create_dataset(name='data', data=data, compression=compression, compression_opts=compression_opts)
...
if __name__ == "__main__":
    ...
    f = create_blank_file(save_file_name='tmp.nwb', force=True)
    scale = DtScale(.1, 'time', 'second')
    data = TimeSeries(y_values,'voltage', 'volt', scale)
    data.add_to_stimulus(f)

    time_series = TimeSeries.get_stimulus(f, 0)
