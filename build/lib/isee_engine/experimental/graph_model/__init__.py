...
def visualize(G, show=True, markersize=1, precision=0, nodelist=None):
    M = nx.adjacency_matrix(G, nodelist=nodelist) # Sparse matrix
    plt.spy(M, markersize=markersize, precision=precision)
    if show == True:
        plt.show()

def from_dict(data_model):
...
def to_dict(G):
    ...
    return {'population_list':population_list, 'connection_list':connection_list}
